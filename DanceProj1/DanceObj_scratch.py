...
class Dance:
    ...
    def get_joint_corr(self, joint1, joint2, label, prominence=.0001, distance=30, sparse=False):
        
        #will look at vertical and horizontal dimensions separately
        #get correlation of between two joints in a given dimension

        moves1 = [joint1.T[i] for i in range(3)] #list of data for each joint in each dimension
        moves2 = [joint2.T[i] for i in range(3)]
    ...
    def get_joint_corr_features(self, sparse):
        ...
        self.get_sacrum()
        nose = [self.acceleration[0], self.jerk[0]]
        Lshoulder = [self.acceleration[3], self.jerk[3]]
        Rshoulder = [self.acceleration[4], self.jerk[4]]
        Lelbow = [self.acceleration[5], self.jerk[5]]
        Relbow = [self.acceleration[6], self.jerk[6]]
        Lwrist = [self.acceleration[7], self.jerk[7]]
        Rwrist = [self.acceleration[8], self.jerk[8]]
        Lhip = [self.acceleration[9], self.jerk[9]]
        Rhip = [self.acceleration[10], self.jerk[10]]
        Lknee = [self.acceleration[11], self.jerk[11]]
        Rknee = [self.acceleration[12], self.jerk[12]]
        Lankle = [self.acceleration[13], self.jerk[13]]
        Rankle = [self.acceleration[14], self.jerk[14]]
        sacrum = [self.sacrum[2], self.sacrum[3]]
        
        joints = [nose, Lshoulder, Rshoulder, Lelbow, Relbow, Lwrist, Rwrist, Lhip, Rhip, Lknee, Rknee, Lankle, Rankle, sacrum]
        jointcorrmap = [[0,0],[1,7],[2,8],[3,4],[5,6],[3,8], [5,10], [4,11], [6,7], [7,12], [11,12], [13,13], [5,13], [6,12]]
        mapnames = ['nose', 'LshoRwri', 'RshoLhip', 'LelbRelb', 'LwriRwri','LelbRhip', 'LwriRkne', 'RelbLankl', 'LhipRwri', 'LhipRankl', 'LanklRankl', 'sacrum', 'LwriSac', 'RwriRankl']
        
        if sparse==True:
            self.get_joint_corr(Rwrist[0], Lwrist[0], 'wrists')
            self.get_joint_corr(Rshoulder[0], Lknee[0], 'contralatRsLk')
            self.get_joint_corr(Rankle[0], Lankle[0], 'ankles')

        
        if sparse==False:
            for i, pair in enumerate(jointcorrmap):
                #get joint correlation features for each pair in jointcorrmap for accel and jerk. for labels, use mapnames + 'accel' or 'jerk'
                self.get_joint_corr(joints[pair[0]][0], joints[pair[1]][0], mapnames[i] + 'accel')
                self.get_joint_corr(joints[pair[0]][1], joints[pair[1]][1], mapnames[i] + 'jerk')
